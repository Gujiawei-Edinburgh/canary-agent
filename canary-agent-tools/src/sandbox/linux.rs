...
const CHILD_LAUNCH_FAILURE_MARKER: &[u8] = b"canary-agent sandbox child launch failed: ";
...
fn create_staging_directory() -> std::io::Result<std::path::PathBuf> {
    let template = CString::new(format!("/tmp/.canary-agent-workspace-{}-XXXXXX", unsafe {
        libc::getpid()
    }))
