...
impl PromWebSearchRecorder {
    ...
    pub fn with_registry_and_labels(
        ...
        let searches = IntCounterVec::new(
            prometheus::Opts::new("lite_agent_web_searches_total", "Web search outcomes")
                .const_labels(labels.clone()),
            &["outcome"],
        )?;
        let durations = HistogramVec::new(
            prometheus::HistogramOpts::new(
                "lite_agent_web_search_duration_seconds",
                "Web search duration",
            )
            .const_labels(labels.clone()),
            &["outcome"],
        ...
        let result_counts = HistogramVec::new(
            prometheus::HistogramOpts::new(
                "lite_agent_web_search_result_count",
        ...
        let response_bytes = HistogramVec::new(
            prometheus::HistogramOpts::new(
                "lite_agent_web_search_response_bytes",
...
mod tests {
    ...
    fn records_web_search_metrics() {
        ...
        assert!(metric_families
            .iter()
            .any(|family| family.get_name() == "lite_agent_web_searches_total"));
        assert!(metric_families
            .iter()
            .any(|family| family.get_name() == "lite_agent_web_search_duration_seconds"));
    }
