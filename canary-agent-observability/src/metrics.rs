use canary_agent_runtime::{MetricsRecorder, RuntimeMetric};
...
impl PromRecorder {
    ...
    pub fn with_registry_and_labels(
        ...
        let turns = IntCounterVec::new(
            prometheus::Opts::new("canary_agent_turns_total", "Agent turn outcomes")
                .const_labels(labels.clone()),
            &["status"],
        )?;
        let turn_latency = HistogramVec::new(
            prometheus::HistogramOpts::new(
                "canary_agent_turn_latency_seconds",
                "Agent turn latency",
            )
            .const_labels(labels.clone()),
            &["status"],
        ...
        let function_calls_per_turn = HistogramVec::new(
            prometheus::HistogramOpts::new(
                "canary_agent_function_calls_per_turn",
        ...
        let model_requests = HistogramVec::new(
            prometheus::HistogramOpts::new(
                "canary_agent_model_request_duration_seconds",
        ...
        let function_calls = HistogramVec::new(
            prometheus::HistogramOpts::new(
                "canary_agent_function_call_duration_seconds",
        ...
        let token_usage = IntCounterVec::new(
            prometheus::Opts::new("canary_agent_tokens_total", "Model token usage")
                .const_labels(labels.clone()),
            &["kind"],
        )?;
        let function_calls_skipped = IntCounterVec::new(
            prometheus::Opts::new(
                "canary_agent_function_calls_skipped_total",
